...
N_bin = 100
hist_counter = r.TH1F('hist_template','neutrino counter',N_bin,0,5)
hist_ETA = r.TH1F('hist_template','neutrino ETA',N_bin,0.0,5.0)
hist_PT = r.TH1F('hist_template','neutrino PT',N_bin,0.0,100.0)
#neutrino number is 22
for i, entry in enumerate(tree):
    ...
    for j in range(len(array_ID)):
        if array_ID[j] == 14 or array_ID[j] == -14 and array_ETA[j] != 0:
...
#Prints number of events with x neutrinos
c = r.TCanvas('neutrino ETA')
hist_ETA.Draw()
c.Print('neutrino_ETA.png')

c2 = r.TCanvas('neutrino counter')
hist_counter.GetXaxis().SetMaxDigits(1)
hist_counter.Draw()
c2.Print('neutrino_counter.png')

c3 = r.TCanvas('Neutrino PT')
hist_PT.Draw()
c3.Print('neutrino_PT.png')
