...
input_file.ls()
hist = r.TH1F('hist','Zmass',100,0,100)
...
for entry in tree:
    ...
    mup_theta = 2*m.arctan(m.e**(-mup_ETA))
    ...
    mum_theta = 2*m.arctan(m.e**(-mum_ETA))
    ...
    mum_E = r.sqrt(mum_p*mum_p + mass_muon*mass_muon)
    ...
    mass= r.sqrt(E*E-px*px-py*py-pz*pz)/1000
